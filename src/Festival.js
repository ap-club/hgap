class Festival extends Phaser.Scene {
  constructor() {
    super('Festival');
  }

  create() {
    const groundHeightPixels = 30;

    /* Scene boilerplate */
    this.bg = this.add.image(0, 0, 'festival');
    this.bg.depth = 0;
    const imgScale = (929 / 829) * (this.scale.height / this.bg.height);
    ...
    this.physics.world.setBounds(0, 0, bgNewWidth, bgNewHeight);
    this.cameras.main.setBounds(0, 0, bgNewWidth, bgNewHeight);
  ...
  play() {
    ...
    const ingredientIndices = [5, 6];
    const ingredientPositionRatios = [487 / 1920, 1594 / 1920];
